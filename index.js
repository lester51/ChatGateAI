...
/*app.get('/getToken', async function(req, res) {
	try {
		...
		let r7 = await axios.post('https:/\/chatgate.ai/wp-json/firebase/v2/users/register-autologin',{
			'user': {
				'userId': data.data.users[0].localId,
				'password': genChars(10),
				'email': data.data.users[0].email,
				'providers': [
					data.data.users[0].providerUserInfo[0].providerId
				]
			}
		}, {
			headers: {
				...
				'cookie': data.cookie,
				'firebase-login-key': data.firebaseKey,
				'origin': 'https://chatgate.ai',
		...
		res.send({
			session_token: btoa(data.cookie + wp_login_cookie)
		})
	} catch (e) {
		if (!e.response)
			res.send({
...
app.get('/getToken', async function(req, res) {
	try {
	 //let {email,cookies} = await axios.get(`http://${require('os').hostname()}:${port}/route1`)
		...
		res.send({data:r6.data,cookie: fullCookie, firebaseKey: firebaseLoginKey})
	} catch (e) {
		if (!e.response)
			res.send({
				error: e.message
